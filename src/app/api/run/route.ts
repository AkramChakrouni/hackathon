...
import { companyProfile, listCompanies, loadQuestionnaires, parseCsv } from "@/lib/corpus";
import type { Event, Question } from "@/lib/types";

export const maxDuration = 60;

export async function POST(req: NextRequest) {
  const body = (await req.json()) as { slug?: string; csv?: string; prospect?: string; company?: string; questions?: Question[] };
  ...
  if (body.slug) {
    ...
    questions = q.questions; prospect ||= q.prospect; company ||= q.company;
  } else if (body.csv) questions = parseCsv(body.csv);
  if (!listCompanies().some((c) => c.slug === company)) company = listCompanies()[0]?.slug ?? "";
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        const r = await runPipeline({ engine: nebiusEngine(), companySlug: company, company: companyProfile(company), prospect, questions, brief: true, signal: req.signal }, emit);
