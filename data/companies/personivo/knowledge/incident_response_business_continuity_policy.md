...
Databases are backed up automatically with a full backup every day and transaction log backups every 10 minutes, allowing point in time restore for 35 days. Backups are encrypted with AES-256, stored geo-redundantly in Azure North Europe (Ireland), and accessible only to two named administrators. File storage uses geo-redundant storage (GRS). Restore tests are performed quarterly and the results are documented.
